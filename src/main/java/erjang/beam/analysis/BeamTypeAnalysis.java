...
import static erjang.beam.CodeAtoms.ERROR_ATOM;
import static erjang.beam.CodeAtoms.EXIT_ATOM;
import static erjang.beam.CodeAtoms.FIELD_FLAGS_ATOM;
import static erjang.beam.CodeAtoms.FLOAT_ATOM;
import static erjang.beam.CodeAtoms.FR_ATOM;
import static erjang.beam.CodeAtoms.F_ATOM;
import static erjang.beam.CodeAtoms.INTEGER_ATOM;
import static erjang.beam.CodeAtoms.LITERAL_ATOM;
import static erjang.beam.CodeAtoms.NIF_ERROR_ATOM;
import static erjang.beam.CodeAtoms.NIL_ATOM;
import static erjang.beam.CodeAtoms.NOFAIL_ATOM;
import static erjang.beam.CodeAtoms.STRING_ATOM;
import static erjang.beam.CodeAtoms.THROW_ATOM;
import static erjang.beam.CodeAtoms.X_ATOM;
import static erjang.beam.CodeAtoms.Y_ATOM;
    ...
    class FV extends FunctionAdapter implements BeamFunction, TypeMap.XRegMarker {
        ...
        private void function_visit_end(List<Integer> dead) {
            ...
            for (LabeledBlock block : this.lbs.values()) {
                ...
                assert (blocks_with_ambiguous_exh.containsKey(block.block_label) == (block_exh != null && block_exh
                    .ambiguousities() != null));
        ...
        class LabeledBlock implements BlockVisitor, BeamCodeBlock {
            ...
            private void accept_2(BlockVisitor2 vis, BeamExceptionHandler exh) {
                ...
                for (int insn_idx = 0; insn_idx < insns.size(); insn_idx++) {
                    ...
                    case gc_bif3: {
                        ...
                        if (srcs.length == 2
                            && (((name == am_plus || name == am_minus) && (rop = srcs[1].testInt()) != null && rop
                                .equals(1))
                    ...
                    case bs_private_append: {
                        ...
                        vis.visitBitStringAppend(opcode, decode_labelref(insn.label, type_map.exh), extra_size, src,
                            unit, flags, dst);
                        break;
                    }
            ...
            private void addExceptionEdge(TypeMap current) {
                ...
                if (handler_lbl >= 0)
                    get_lb(handler_lbl, false).merge_from(current);
            }
                ...
                if (initial == null) {
                    ...
                    if (new_types == initial) {
                        ...
                        if (log.isLoggable(Level.FINE))
                            log.fine("Missed TypeMap sharing opportunity");
                    } else {
